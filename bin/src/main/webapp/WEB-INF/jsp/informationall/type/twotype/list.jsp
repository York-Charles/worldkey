...
<table id="tb" class="table table-striped">
    ...
    <c:forEach items="${list }" var="c" varStatus="co">
        <tr>
            ...
            <td>
             <!--  4.11薛秉臣 添加三级分类  -->
             <span class="find"  id="${c.id}"><a href="type/three/list/${c.id}">分类</a></span>
            
            <span class="compile" id="${c.id}"> <img alt=""  src="icon/icon/icon_btn_write.png">
				</span> <span class="del" id="${c.id}"> <img alt=""  src="icon/icon/icon_btn_cancel.png">
				</span></td>
            <!--  <td>关闭排序功能4.19
                <c:if test="${!co.first}">
...
<div id="addtype" class="input">
    <form action="type/two/add" method="post" target="new">
        <input type="hidden" name="oneType" value="${oneType}">
...
<script>
    $(function () {
        $(".replace").click(function () {
            ...
            $.post("/type/replace", {id: id, replace: replace}, function (data) {
                if (data === 1) {
                    location.href = "/type/two/list/${oneType}";
                } else {
                    alert(data);
                }
            });
        ...
        $(".del").click(function () {
            if (window.confirm("确定删除?")) {
                ...
                $.post('type/two/del/' + $(this).attr('id'), function (data) {
                    if (data.msg == "ok") {
                        obj.remove();
                        alert("删除成功");
                    } else {
        ...
        $(".compile").click(function () {
            if (compileType === 1) {
                ...
                if (window.confirm("确定修改?")) {
                    ...
                    $.post("type/two/update?id=" + $(this).attr('id') + "&typeName=" + name, function (data) {
                        if (data.msg !== "ok") {
                            alert(data.result);
                        }
                    });
