...
public class InformationAllController {
	...
	public Object ajaxUpdate(@RequestHeader("host") String host, String info, String title, String abstracte, Long id,
			String titleImg, MultipartFile file, String oldTitleImg) {
		String result = "no";
	 ...
	 * 
	 * @RequiresRoles("informationall") public @ResponseBody ResultUtil
	 * changeTwoType(Integer changeId, Integer changeTwoType) { String typeName
	 * = this.allService.changeTwoType(changeId, changeTwoType);
	 * log.debug("changeTwoType:{};changeId:{}", changeTwoType, changeId);
	 * log.debug("typeName:{}", typeName); return new ResultUtil(200, "ok",
	 * typeName); }
	 */
	...
	public ResultUtil tuijian(@RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
			@RequestParam(value = "pageSize", defaultValue = "10") Integer pageSize, @NotNull String token)
			throws Exception {
	...
	public ModelAndView list(@RequestParam(value = "page", defaultValue = "1") Integer pageNum,
			@RequestParam(value = "pagesize", defaultValue = "10") Integer pageSize, InformationAll vo,
			@RequestParam(defaultValue = "1") Integer onetype, Integer twotype,
			@RequestParam(defaultValue = "10000") Integer threetype, @RequestHeader("host") String host) {
		ModelAndView model = new ModelAndView("informationall/list");
		log.info(threetype + "11111111111111111111111111111111111111");
...
//			page = this.allService.findByType(pageNum, pageSize, threetype, host);
			page = this.allService.findByType(pageNum, pageSize, threetype, host,vo.getChecked(),vo.getTitle());
	...
	// @ResponseBody
	// public ResultUtil findByType1(@RequestParam(value = "page", defaultValue
	// = "1") Integer pageNum,
	// @RequestParam(value = "pagesize", defaultValue = "10") Integer pageSize,
	...
	// this.allService.findBySelective1(pageNum, pageSize, vo,token));
	// }
	...
	public ModelAndView update(@Validated InformationAll vo, Errors errors, MultipartFile file,
	...
	/*
	 * 跳转到更新页面，并做一些预处理 2018.4.19 暂时关闭修改编辑页面
	 * 
	...
	// giftRecordService.findGiftRecordByToInformation(id);
	// return new ResultUtil(200, "ok", list);
	// }

	@RequestMapping("giftRecord")
	@ResponseBody
	public ResultUtil giftRecord(Long id) {
