...
public class InformationController {
	...
	/**
	 * 创建状态： id:自增 title:新建文章 checked:2 type:null
	 * 
	 * @param vo
	 * @param host
	 * @param session
	...
	/*
	 * @RequestMapping(value = "update/{id}") public String update(Model
	 * model, @PathVariable Long id, HttpSession session) { Users users =
	 * (Users) session.getAttribute("users"); InformationExample info =
	 * this.allService.getAllMapper().selectById(id); if
	 * (!(users.getId().equals(info.getUser()))) { return
	 * "redirect:/users/information/list"; } model.addAttribute("info",
	 * this.allService.info(id)); TwoType byId =
	 * this.twoTypeService.findById(info.getType());
	 * model.addAttribute("twoType", byId); model.addAttribute("oneType",
	 * this.oneTypeService.findById(byId.getOneType())); return
	 * "usersM/information/updateWrite"; }
	 */
	/*
	 * 以下2018.4.16更新
	 */
	// @RequestMapping(value = "update/{id}")
	// public String update(Model model, @PathVariable Long id, HttpSession
	// session) {
	// Users users = (Users) session.getAttribute("users");
	// InformationExample info =
	// this.allService.getAllMapper().selectById(id);//根据Id查询InformationAll的某条信息(含内容)
	// if (!(users.getId().equals(info.getUser()))) {
	// return "redirect:/users/information/list";
	// }
	// TwoType byId = null;
	// model.addAttribute("info", this.allService.info(id));//
	// 增加点击次数，根据传入Id选择InformationAll的一条数据
	// ThreeType byId2 =
	// this.threeTypeService.findById(info.getType());//根据该info的type查询到threeType表中对应的type
	// if (byId2 != null) {//验证是否存在
	// byId =
	// this.twoTypeService.findById(byId2.getTwoType());//若存在，则判断info的type对应TwoType的type
	// model.addAttribute("threeType", byId2);
	// } else {
	// byId = this.twoTypeService.findById(info.getType());
	// }
	// model.addAttribute("twoType", byId);
	// model.addAttribute("oneType",
	// this.oneTypeService.findById(byId.getOneType()));
	...
	public ModelAndView updateType(@Validated @ModelAttribute("information") InformationExample vo, Model model,
			HttpSession session) {
