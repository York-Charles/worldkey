...
public class TypeController {
	...
	@ResponseBody
	public ResultUtil findTwoByOne2(@PathVariable Integer one) {
	...
	@RequestMapping("two")
	@ResponseBody
	public ResultUtil findTwoByOne1(Integer one) {
		return new ResultUtil(200, "ok", this.twoTypeService.findByOne(one));
	}
	...
	@ResponseBody
	public ResultUtil findThreeByTwo2(@PathVariable Integer two) {
	...
	@ResponseBody
	public ResultUtil findThreeByTwo1(Integer two) {
		return new ResultUtil(200, "ok", this.threeTypeService.findByTwo(two));
	}
	//五板块图片
	@RequestMapping("iconTwo")
	@ResponseBody
	public ResultUtil iconTwo(Integer one) {
		return new ResultUtil(200, "ok", this.threeTypeService.findByOne(one));
	}
...
//	}
	...
	public ResultUtil addGroup(String token, HttpServletRequest request) throws IOException {
		MultipartHttpServletRequest params = ((MultipartHttpServletRequest) request);
		MultipartFile headImgFile = params.getFile("headImg");
		MultipartFile bgImgFile = params.getFile("bgImg");
		String typeName = params.getParameter("typeName");
...
//	}
	@RequestMapping(value="group/updateDetail", method = RequestMethod.POST)
	public ResultUtil updateGroup(String token, Integer groupId, HttpServletRequest request,String content) throws IOException{
		Users user = this.uService.findByToken(token);
		Long userId = this.threeTypeService.selectLeaderId(groupId);
		MultipartHttpServletRequest params = ((MultipartHttpServletRequest) request);
		MultipartFile headImgFile = params.getFile("headImg");
...
//	}
	...
	public ResultUtil addMember(String token, Integer groupId) {
		Users user = this.uService.findByToken(token);
		UserGroup userGroup = new UserGroup(user.getId(), groupId);
		//加入唯一小组		
				List<Integer> l = this.ugMapper.SelectExistence(user.getId());
...
//	}
	@RequestMapping(value = "group/delMember", method = RequestMethod.GET)
	public ResultUtil delMember(String token, Integer groupId) {
		Users user = this.uService.findByToken(token);
		Long userId = this.threeTypeService.selectLeaderId(groupId);
		if(userId.equals(user.getId())){
			return new ResultUtil(500, "error", "您是组长，不能退出！");
    ...
    public ResultUtil number(Integer userId){
    	Integer a = this.ugMapper.threeNumber(userId);
    	Integer b = this.ugMapper.number(userId);
    	Integer c = b - a;
		return new ResultUtil(200,"ok",c);
    }
