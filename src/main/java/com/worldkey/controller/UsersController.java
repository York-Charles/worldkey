...
public class UsersController {
     ...
     */
    @RequestMapping("getJdAndZsAndKb/{token}")
    ...
    @RequestMapping("getJdAndZsAndKb")
    public ResultUtil getJdAndZsAndKb1(String token) {
        Users byToken = this.uService.findByToken(token);
        if (byToken == null) {
            return new ResultUtil(406, "no", "未登录");
        }
...
//    }
    ...
    public ResultUtil checkLoginName(String loginName) {
        int s = this.uService.checkName(loginName);
        return new ResultUtil(200, "ok", s);
    }
    
    @RequestMapping("checkPetName")
    public ResultUtil checkPetName(String petName) {
        int s = this.uService.checkPetName(petName);
        return new ResultUtil(200, "ok", s);
    }
...
//    }
    ...
    public ResultUtil giveGift(String token, Integer giftId, Long toInformation) {
        Users byToken = uService.findByToken(token);
        if (byToken == null) {
            return new ResultUtil(406, "no", "未登录");
        }
