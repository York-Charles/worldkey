...
public interface ApplyMapper extends BaseMapper<Apply> {
	...
	// 根据物品ID查询申请数，集合
	@Select("SELECT apply_id,users,information,status,head_img AS headImg ,create_time,users.pet_name AS petName  FROM apply,users WHERE (information=#{information} AND status='1' AND apply.users=users.id)")
	...
	// 乐赠闲置形成的订单表
	@Select("SELECT apply_record_id,users_seller,information,users_buyer,head_img AS headImg,create_time,msg,users.pet_name FROM apply_record,users WHERE (information=#{information} AND apply_record.users_buyer=users.id)")
