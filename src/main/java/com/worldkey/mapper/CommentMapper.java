...
public interface CommentMapper {
    ...
    @Select("select users.pet_name from users left JOIN comment on users.id =comment.users where comment.comment_id=#{comment}")
    String selectMyPetName(Long comment);
  ...
  //5.18
    @Select("SELECT comment.information,comment.gmt_create,comment.info,comment.comment_id "+
    		"from comment "+
    ...
    @Select("select comment_id from comment where information=#{id} ORDER BY gmt_create DESC")
    List<Long> location(Long information);
    
    @Select("select information from comment where comment_id=#{id}")
    Long information(Long commentId);
}