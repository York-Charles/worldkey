...
public interface FriendMapper {
	...
	@Select("select record.id,record.to_user_id,record.user_id,record.message,record.create_time,record.type,users.pet_name AS petName,users.login_name AS loginName,users.head_img AS headImg,users.signature AS signature,users.sex AS sex from record left join users on users.login_name = record.user_id where record.to_user_id=#{toUserId} order by record.create_time desc")
	...
	@Delete("delete from record where (user_id = #{userId} and to_user_id = #{toUserId}) or (user_id = #{toUserId} and to_user_id = #{userId})")
	int delAsk(@Param("userId")String userId,@Param("toUserId")String toUserId);
	
	@Delete("delete from record where to_user_id=#{toUserId} and user_id=#{userId}")
	void delSingle(@Param("toUserId")String toLoginName,@Param("userId")String loginName);
}