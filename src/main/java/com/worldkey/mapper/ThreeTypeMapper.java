...
public interface ThreeTypeMapper extends BaseMapper<ThreeType> {
	...
	/**
	 * 根据yi级Type获取er级标签的内容（说说同样适用）
	 * @param id
	 * @return
	 */
	@Select("select id,type_name as typeName,type_img as typeImg,one_type as oneType from two_type where one_type=#{id} limit 0,8")
	List<TwoType> selectByOne(@Param("id") Integer id);
	...
	*/
	...
	//5.11
		...
		@Select("select id, type_name, two_type, head_img, bg_img, content,user_id,amount "+
				 "from three_type where user_id=#{userId} and checked=1")
