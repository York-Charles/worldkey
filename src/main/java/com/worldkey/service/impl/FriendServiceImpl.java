...
public class FriendServiceImpl extends ServiceImpl<GiftMapper, Gift> implements FriendService {
	...
	public Map<String, Object> elopeHandler(Users user, String type, String message, String... toUserId) {
		...
		String fromUserId = user.getLoginName();
		CodeSuccessResult messagePublishPrivateResult = null;
		CustomizeElope friendMessage1 = new CustomizeElope(this.cbum.selectLabelByUserId(user.getId().toString()), type,
				user.getId().toString());
		...
		try {
			messagePublishPrivateResult = rongCloud.message.publishPrivate(fromUserId, toUserId, friendMessage1, "私奔请求",
					"{\"pushData\":\"hello\"}", "4", 0, 0, 0, 0);
	...
	public PrivateRequest infoCheck(String reqName, String message, String... toUserId) {
		...
		// rongCloud.message.publishPrivate(reqName, toUserId, pr,
	...
	public void infoCheck1(Users user, Users user1) {
		...
		try {
			messagePublishPrivateResult = rongCloud.message.publishPrivate(user.getLoginName(), s, pr, "私奔请求",
					"{\"pushData\":\"hello\"}", "4", 0, 0, 0, 0);
	...
	public void dissPeople(Users user, Users user1) {
		...
		String[] s = { user1.getLoginName() };
		try {
			messagePublishPrivateResult = rongCloud.message.publishPrivate(user.getLoginName(), s, dis, "私奔请求",
					"{\"pushData\":\"hello\"}", "4", 0, 0, 0, 0);
	...
	public void barrage(String userName, String message, String icon, String type, String... toUserId) {
		...
		if (type.equals(FriendServiceImpl.NOTICE)) {
			...
			try {
				messagePublishPrivateResult = rongCloud.message.publishPrivate(userName, toUserId, n, "私奔请求",
						"{\"pushData\":\"hello\"}", "4", 0, 0, 0, 0);
			...
			try {
				messagePublishPrivateResult = rongCloud.message.publishPrivate(userName, toUserId, b, "私奔请求",
						"{\"pushData\":\"hello\"}", "4", 0, 0, 0, 0);
				log.info(messagePublishPrivateResult.toString() + ">>>>>>>>>>>>>>>>>>>>>>>");
			} catch (Exception e) {
				e.printStackTrace();
	...
	public Integer giftUsers(String token, Integer giftId, Long userId, String... toUserId) {
		...
		if (jdPrice && jdAndZsAndKb.getJd() >= gift.getJdPrice()) {
			...
			JdDetail jdDetail = new JdDetail(new Date(), gift.getJdPrice() * -1, JdRewardType.giveGift.getIndex(),
					users.getId(), userId, users1.getPetName(), "送给" + users1.getPetName() + "一个" + gift.getName());
			// 受赠人的收入记录
			JdDetail jdDetail1 = new JdDetail(new Date(), gift.getJdPrice(), JdRewardType.giveGift.getIndex(), userId,
					users.getId(), users.getPetName(), "收到" + users.getPetName() + "送的一个" + gift.getName());
	...
	public List<FriendExample> findFriend(String loginName) {
		...
		// systemConfigService.find().getDefaultHeadimg();
		// list.stream().filter(e -> e.getHeadimg() == null).forEach(e ->
		// e.setHeadimg(defaultHeadImg));
		return list;
	}
	...
	private Integer Bestowed(boolean insert, Users users, Users users1, Integer giftId, String... toUserId) {
		if (insert) {
			...
			BestowUtilCustomize best = new BestowUtilCustomize(200, "ok",
					new BestowUtil(s, this.cbum.selectSeatByUserId(users.getId()),
							this.cbum.selectSeatByUserId(users1.getId()), this.gm.getGiftImg(giftId)));
			best.setContent(GsonUtil.toJson(best, BestowUtilCustomize.class));
			try {
				messagePublishPrivateResult = rongCloud.message.publishPrivate(users.getLoginName(), toUserId, best,
						"私奔请求", "{\"pushData\":\"hello\"}", "4", 0, 0, 0, 0);
	 ...
	 */
	@Override
	public void delSingle(String toLoginName,String loginName){
		this.friendMapper.delSingle(toLoginName, loginName);
	}
