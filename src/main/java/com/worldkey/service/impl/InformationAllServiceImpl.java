...
public class InformationAllServiceImpl implements InformationAllService {
	...
	static List<Integer> xl=Arrays.asList(10034,10035,10036,10037,10038,10039,10040,10041,10042,10043,10044,10107,10108,10109,10110,10111,10112);
	 ...
	 */
	@Override
	public PageInfo<InformationAll> findByType(Integer pageNum, Integer pageSize, Integer type,String host,Integer checked,String title) {
		PageHelper.startPage(pageNum, pageSize, true);
//		List<InformationAll> list = this.allMapper.selectByType(type);
		List<InformationAll> list = this.allMapper.selectByType(type,checked,title);
...
//	@Cacheable(value = "info", key = "'InformationAll'+#itemID")
	...
	public PageInfo<BaseShow> selectByOneType(Integer type, Integer pageNum, Integer pageSize, String token) {
		...
		for(BaseShow baseShows1:baseShows){
			if(xl.contains(baseShows1.getType())==true){
				baseShows1.setXl(1);
			}
		}
		if(token!=null){
	...
	@Override
	public InformationAll as(Long id) {
		
		return this.informationAllMapper.as(id);
	}
