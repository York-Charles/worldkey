...
<body>
	...
	<!-- 	<c:if test="${threeTypes.size()!=1}">
		...
		<form style="float: right;" class="form-inline">
			<input type="hidden" name="oneType" value="${oneType.id}"> <input
				type="text" placeholder="search" value="${q}" name="q" id="q"
				class="form-control"
	...
	<table class="table table-hover table-striped text-center">
		...
		<c:forEach items="${pageInfo.list}" var="item" varStatus="c">
			<tr>
				<td>${c.count}</td>
				<td>
					<%-- 没有标题的展示 --%> <c:if test="${item.title==null}">
						<a href="${item.webUrl}">无标题${item.id}</a>
					</c:if> <%-- 有标题的展示 --%> <c:if test="${item.title!=null}">
						<a href="${item.webUrl}">${item.title}</a>
					</c:if>
				</td>
				<td>${oneType.typeName}<c:forEach items="${threeTypes}"
						var="typeItem">
						<c:if test="${typeItem.id==item.type}">${typeItem.typeName}</c:if>
					</c:forEach>
				</td>
				<td>
                ...
                -->
					...
					<button type="button" data-id="${item.id}"
						class="change btn-sm  btn btn-warning" data-toggle="modal"
						data-target="#move">移动</button>
	...
	<nav aria-label="Page navigation" class="text-center">
		<ul class="pagination">
			<c:if test="${!pageInfo.isFirstPage}">
				<li><a class="page " data-id="${pageInfo.prePage}"
					aria-label="Previous"> <span>上一页</span>
				</a></li>
			</c:if>
			<%--@elvariable id="pageinfo" type="com.github.pagehelper.PageInfo"--%>
			<c:forEach
				...
				<c:if test="${pageInfo.pageNum==i }">
					<li class="active"><span class="page" data-id="${i}">${i}</span></li>
				</c:if>
				<c:if test="${pageInfo.pageNum!=i }">
					<li><a class="page" data-id="${i}"><span> ${i}</span></a></li>
				</c:if>
			</c:forEach>

			<c:if test="${!pageInfo.isLastPage}">
				<li><a class="page" data-id="${pageInfo.nextPage}"
				...
				<div class="modal-body">
					<select id="selectOneType" name="oneType" title="">
						...
						<c:forEach items="${oneTypes}" var="item">
							<c:if test="${item.id!=33&&item.id!=34 }">
								<option value="${item.id}">${item.typeName}</option>
							</c:if>
						</c:forEach>
					</select> <select name="type" id="type" title="">
						<option value="0">请选择</option>
					</select> <select name="three" id="three" title="">
						<option value="0">请选择</option>
					</select>
				</div>
				<div class="modal-footer">
					<button type="button" id="qx" class="btn btn-default"
